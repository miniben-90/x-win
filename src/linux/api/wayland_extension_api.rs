...
use crate::{
  ...
  linux::api::gnome_shell::{
    value_to_window_info, GNOME_XWIN_COMMON_FN, GNOME_XWIN_EXTENSION_FOLDER_PATH, GNOME_XWIN_EXTENSION_META, GNOME_XWIN_EXTENSION_SCRIPT, GNOME_XWIN_UUID
  },
};
...
{}
  "#,
    GNOME_XWIN_COMMON_FN, GNOME_XWIN_EXTENSION_SCRIPT
  );
  if fs::write(get_extension_file_path(), content).is_ok() {
    if fs::write(get_medata_file_path(), GNOME_XWIN_EXTENSION_META).is_ok() {
      ...
      //     ()
      //   } else {
      //     // cleanup_extension_folder().unwrap();
      //     panic!("Extension is not enabled!");
      //   }
      // }
      ()
    } else {
